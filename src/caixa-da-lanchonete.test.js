...
describe("CaixaDaLanchonete", () => {
  ...
  test("Verifica Forma de pagamento inválida", () => {
    expect(caixa.calcularValorDaCompra("pix", ["cafe,1"])).toBe(
      "Forma de pagamento inválida!"
    );
  });

  test("Verifica Item inexistente", () => {
    expect(caixa.calcularValorDaCompra("debito", ["cappuccino,1"])).toBe(
      "Item inválido!"
    );
  });

  test("Verifica Não há itens no carrinho", () => {
    expect(caixa.calcularValorDaCompra("dinheiro", [])).toBe(
      "Não há itens no carrinho de compra!"
    );
  });

  test("Verifica item extra sem o principal", () => {
    expect(caixa.calcularValorDaCompra("debito", ["chantily,1"])).toBe(
      "Item extra não pode ser pedido sem o principal"
    );
  });
});
